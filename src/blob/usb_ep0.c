...
/*		SerialOutputString("req != request\n"); */
	...
	if ( request_type != 0 ) {
		if (usb_debug) SerialOutputString("unsupported bmRequestType\n");
		goto sh_sb_end;
	}
...
/*			SerialOutputString("GET_DESCRIPTOR\n"); */
		...
		case SET_INTERFACE:
			if (usb_debug) SerialOutputString("SET_INTERFACE\n");
			break;

		case SET_DESCRIPTOR:
			if (usb_debug) SerialOutputString("SET_DESCRIPTOR\n");
			break;

		case SET_CONFIGURATION:
			if (usb_debug) SerialOutputString("SET_CONFIGURATION\n");
/*
...
*/
	...
	if ( bytes_this_time!=0 ) {
		volatile unsigned char* pReg = (volatile unsigned char*)&UDCDR0;
...
//			SerialOutputString("UDCCSR0=");SerialOutputDec(UDCCSR0);SerialOutputString("\n");
			...
			if ( (UDCCSR0&SETUP_READY) == UDCCSR0_OPC ) {
				...
				SerialOutputString("Next Setup command arrive.\n");
				return;
			}
#endif
...
//				SerialOutputString("Clear OPC.\n");
		...
		if ( count==0 ) {
			if (usb_debug) SerialOutputString("waiting for host's ack. But time out.\n");
		}
	}
/*	msleep(1);*/
