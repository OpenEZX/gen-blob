...
static void handle_command(u8 *command_ptr, u8 *data_ptr)
{
    ...
    switch(map_command(&command_tbl[0], num_elements(command_tbl), command_ptr))
    {
        ...
        else if (*pCurrentPtr == RS)
        {
            ...
            else    /* is BIN command */
            {
                ...
                else
                {
                    ...
                    BIN_bytes_to_read = ( (recvData[BIN_DATA_SIZE_MSB]
                                             << SHIFT_MSB) + 
                                             (recvData[BIN_DATA_SIZE_LSB]) );

                    /* check for a valid data packet size */
                    if ( (BIN_bytes_to_read < MIN_BIN_PACKET_SIZE) 
                                  || (BIN_bytes_to_read > MAX_BIN_PACKET_SIZE) )
                    {
                    ...
                    else    /* data size is ok */
                    {
                         ...
                         */
                        BIN_bytes_to_read += (MAX_DATA_FIELD_SIZE + 2);

                        /* has read all of the data */
                        if (data_offset >= BIN_bytes_to_read)   
                        {
                            /* check that BIN command was good */
                            if (recvData[BIN_bytes_to_read - 1] == ETX )
                            {
                                recvData[BIN_bytes_to_read - 1] = NUL;
                                handle_command(recvCmd, recvData);
                                reset_global();
                                return;
                            }
                            else
...
==============================================================================*/
void
parse_err_response(u8 error_code)
{
    u8 error_code_str[2];
