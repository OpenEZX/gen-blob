...
get_fatent(fsdata *mydata, __u32 entry)
{
	...
	case 32:
		...
		ret = FAT2CPU32( (((__u16*)mydata->fatbuf)[offset*2]) | (((__u16*)mydata->fatbuf)[offset*2+1] << 16) );
		break;
	case 16:
	...
	do {
		...
		while(actsize < filesize) {
			...
			if((newclust -1)!=endclust)
				goto getit;
			if (mydata->fatsize < 32 && (newclust <= 0x0001 || newclust >= 0xfff0)) {
				FAT_DPRINT("\ncurclust: 0x%x\n", newclust);
...
getit:
		...
		if (mydata->fatsize < 32 && (curclust <= 0x0001 || curclust >= 0xfff0)) {
			FAT_DPRINT("\ncurclust: 0x%x\n", curclust);
