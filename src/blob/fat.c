...
int fat_register_device(block_dev_desc_t * dev_desc, int part_no)
{
	...
	if (dev_desc->block_read(dev_desc->dev, 0, 1, buffer) != 1) {
...
static __u32 get_fatent(fsdata * mydata, __u32 entry)
{
	...
	case 32:
		...
		ret =
		    FAT2CPU32((((__u16 *) mydata->fatbuf)[offset *
							  2]) | (((__u16 *)
								  mydata->
								  fatbuf)[offset
									  * 2 +
	...
	do {
		...
		while (actsize < filesize) {
			...
			if (mydata->fatsize < 32
			    && (newclust <= 0x0001 || newclust >= 0xfff0)) {
				FAT_DPRINT("\ncurclust: 0x%x\n", newclust);
		...
		if (mydata->fatsize < 32
		    && (curclust <= 0x0001 || curclust >= 0xfff0)) {
			FAT_DPRINT("\ncurclust: 0x%x\n", curclust);
...
do_fat_read(const char *filename, void *buffer, unsigned long maxsize, int dols)
{
	...
	if (mydata->fatsize == 32) {
		...
		rootdir_size =
		    ((bs.dir_entries[1] * (int)256 + bs.dir_entries[0])
