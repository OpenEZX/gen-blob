...
#ifdef MMC_DEBUG
	debug("R%02d: %08X %08X\n     %08X %08X\n", cmd, resp[0], resp[1], resp[2], resp[3]);
#endif // MMC_DEBUG
	return 0;
}
...
//	MMC_CLKRT  = MMC_CLKRT_0_3125MHZ;
	...
	strncpy(mmci.blkdev.product, mmci.cid.prod_name, sizeof(mmci.blkdev.product));
	sprintf(mmci.blkdev.revision,
		 "%x %x", mmci.cid.hwrev, mmci.cid.hwrev);
	mmci.blkdev.dev = 0;
	  ...
	  unsigned char *buffer)
{
	...
	ret = mmc_cmd(MMC_CMD_SET_BLOCKLEN, mmc->blkdev.blksz, MMC_CMDAT_R1, &resp);
	if (ret) goto fail;
...
fail:
	...
	mmc_cmd(MMC_CMD_SEND_STATUS, mmc->rca << 16, &card_status, MMC_CMDAT_R1);
